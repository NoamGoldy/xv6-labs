...
struct {
  struct spinlock lock;
  struct buf buf[NBUF];
  struct chache_bucket buckets[NBUCKET];
} bcache;
...
bget(uint dev, uint blockno)
{
  ...
  struct chache_bucket *bucket = &bcache.buckets[blockno % NBUCKET];
  acquire(&bucket->lock);
...
brelse(struct buf *b)
{
  struct chache_bucket *bucket = &bcache.buckets[b->blockno % NBUCKET];
  if(!holdingsleep(&b->lock))
    panic("brelse");
